...
{
    OutputDebugStringW(L"Profiler.dll!Profiler::Initialize\n");
    ...
    DWORD eventMask = COR_PRF_MONITOR_JIT_COMPILATION | COR_PRF_MONITOR_CACHE_SEARCHES;

    // Uncomment and it will stop crashing
    // eventMask |= COR_PRF_MONITOR_CODE_TRANSITIONS;

    pCorProfilerInfo->SetEventMask2(eventMask, COR_PRF_HIGH_MONITOR_NONE);
...
{
    OutputDebugStringW(L"Profiler.dll!Profiler::Shutdown\n");
...
{
    const auto functionName = GetFunctionIDName(functionId);

    if (functionName == L"InvokePreStartInitMethodsCore")
    {
        ...
        HRESULT hr = pCorProfilerInfo->GetFunctionInfo2(functionId,
            NULL,
            &classId,
            &moduleId,
            &token,
        ...
        if (FAILED(hr))
        {
            OutputDebugStringW(L"FAIL: GetFunctionInfo2 call failed");
            return hr;
        }

        pCorProfilerInfo->ApplyMetaData(moduleId);
    }
...
{
    auto functionName = GetFunctionIDName(functionId);
